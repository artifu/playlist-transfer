...
const SAFE_EVENT_PROPERTY_KEYS = new Set(SAFE_ANALYTICS_PROPERTY_KEYS);
...
async function handleAppleMusicCatalogSearch(env, request) {
  ...
  if (term.length < 2) {
    throw new Error("Enter at least two characters to search Apple Music.");
  }

  const results = await createAppleMusicClient(env).searchSongs(term, limit);
  return noStoreJson(200, { results });
}
...
export async function handleNativeApiRequest(context) {
  ...
  try {
    ...
    if (method === "POST" && path === "/api/apple-music/catalog-search") {
      return await handleAppleMusicCatalogSearch(context.env, request);
    }
